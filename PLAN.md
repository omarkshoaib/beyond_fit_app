...
2. ~~**`httpOnly` cookie or secure mobile-only storage on web.**~~ ✅ Done. `app/auth/deps.py:get_current_user` now resolves user from either an `Authorization: Bearer …` header (native) or an `access_token` cookie (web). `/auth/login`, `/auth/register`, `/auth/refresh` all set `httpOnly` cookies via `_set_auth_cookies`. New `/auth/logout` endpoint clears them. `COOKIE_SECURE=true` env var flips Secure + SameSite=Strict for prod (default `false` for local HTTP). New tests: `test_login_sets_httponly_cookies_and_cookie_auth_works`. Native mobile keeps reading the JSON body (Bearer token); the web build can opt into cookie auth with no client-side change to the API client (Dio's default browser adapter forwards cookies automatically). Migration to a fully cookie-only web session is now a one-line config change.
...
4. ~~**Email verification at registration.**~~ ✅ Done. New `verified_at` field on `ClientProfile` (alembic 0013). Register now fires `EmailService.send_verification` (deep-link to `/verify?token=…`). New endpoints `POST /auth/verify` (consumes token, stamps `verified_at`) and `POST /auth/resend-verification` (auth required). New mobile screen `VerifyEmailScreen` with success/failure states. Home renders an amber "Verify your email" banner with Resend button when `verified_at == null`. `auth/me` now returns `verified_at`. Tests: 3 added covering verify, resend, garbage rejection. Note: critical actions are *not* yet gated on verification — that's an intentional follow-up (we want users to onboard immediately, then nag them to verify).
...
8. ~~**App icons + splash screens.**~~ ✅ Done. `mobile/assets/icon_source.png` is a 1024×1024 gradient + lightning-bolt mark generated by PIL. `flutter_launcher_icons` + `flutter_native_splash` configured in `pubspec.yaml`; both regenerated all platform variants (Android adaptive icons, iOS app icons, splash on Android 12+).
9. ~~**Deep links / universal links.**~~ ✅ Done. Android `AndroidManifest.xml` now has `android:autoVerify="true"` intent-filters for `https://beyondfit.app` + a custom `beyondfit://` scheme. iOS `Info.plist` registers the custom scheme; new `Runner.entitlements` declares `applinks:beyondfit.app`. `lib/main.dart` listens to both cold-start and warm-start URIs via `app_links` and routes them to the matching GoRouter location. Pre-flight metadata files in `docs/well-known/` (`apple-app-site-association` + `assetlinks.json`) ready to host on the production domain — see `docs/well-known/README.md` for the deployment checklist (replace `TEAMID` and the SHA-256 fingerprint).
10. ~~**App-store metadata.**~~ ✅ Done. `docs/PRIVACY_POLICY.md`, `docs/TERMS.md`, and `docs/STORE_LISTING.md` cover privacy disclosures (GDPR/CCPA), Terms of Service, and full Apple + Google submission copy (short / long descriptions, keywords, age rating, screenshot specs, App Privacy answers, Play Data Safety section). All `[bracketed placeholders]` flagged for replacement before submission.
